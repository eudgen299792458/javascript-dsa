function areThereDuplicateArgs(...args) {
  args.sort((a, b) => a - b);
...
function areThereDuplicateByMultiplePointers(...args) {
  args.sort((a, b) => a - b);
  let pointer1 = 0;
  let pointer2 = 1;
  for (let index = 0; index < args.length - 1; index++) {
...
function testAreThereDuplicateByMultiplePointers() {
  const testCases = [
    // Simple Test Cases
  ...
  console.log("------ areThereDuplicateArgs test case -----------");
  testCases.forEach(({ params, expected }, index) => {
    const result = areThereDuplicateArgs(...params);
    if (result === expected) {
  ...
  console.log(
    "------ areThereDuplicateByMultiplePointers test case -----------"
  );

  testCases.forEach(({ params, expected }, index) => {
    const result = areThereDuplicateByMultiplePointers(...params);
    if (result === expected) {
      console.log(`Test case ${index + 1}: Passed ✅`);
    } else {
      console.log(`Test case ${index + 1}: Failed ❌`);
...
// Run the test suite
testAreThereDuplicateByMultiplePointers();
